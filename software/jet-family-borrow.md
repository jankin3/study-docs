...
### 1. 配置破解补丁
编辑配置文件 `xxx.vmoptions`文件
在最后加上类似
```python
-javaagent:/Users/xxx/workspace/data/jetbrains-agent/jetbrains-agent.jar
```
文件下载位置:http://jankin.club/jetbrains-agent.zip

注意：有一次从命令行编辑失效，但是使用然后ide中编辑生效
